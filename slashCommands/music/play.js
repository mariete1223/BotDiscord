...
module.exports = {
    name: "play",
    description: "Plays Music in your Voice Channel(VC)",
    aliases: ["p"],
    options:[
        {
            name: "songtitle",
            description: "Title/Link of song oor playlist",
            type: "STRING",
            required: true,
        },
    ],
    run: async (client,interaction,args,prefix) => {
        ...
        if(!oldConnection){
           ...
           //if there is queue but not an old connection we delete it
            ...
            interaction.reply({content: `**❌ Couldn't play that in your VC due to:  \`\`\` ${String(e?.interaction || e).substring(0,1900)} \`\`\` `}).catch(()=> null);
        }
